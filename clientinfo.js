...
(function() {
	...
	window.clientinfo = function () { 
		...
		var osv = function(){
			...
			else if(os == 'windows') return /windows (.*?)\;/i.exec(av)[1]; // somehow firefox does get the correct os version (the regex works), but doesnt return the value here.
...
window.onload=function(){
	...
	document.getElementById('clientinfo').innerHTML += JSON.stringify(clientinfo);
